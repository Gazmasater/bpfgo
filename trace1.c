...
int trace_accept4_enter(struct sys_enter_accept4_args *ctx) {
    ...
    if (ctx->upeer_sockaddr != NULL) {
        struct sockaddr_in *addr = ctx->upeer_sockaddr;
...
int trace_accept4_exit(struct sys_exit_accept4_args *ctx) {
    ...
    struct sockaddr_in **addr_ptr = bpf_map_lookup_elem(&sockaddr_map, &pid);
    ...
    if (addr.sin_family == AF_INET) {
            ...
            bpf_printk("sys_exit_accept4:ADDR=%d.%d.%d.%d:%d PID=%d, Comm=%s\n",
...
int trace_connect_enter(struct sys_enter_connect_args *ctx) {
    ...
    if (ctx->uservaddr != NULL) {
        struct sockaddr_in *addr = ctx->uservaddr;
    
        bpf_map_update_elem(&sockaddr_map, &pid, &addr, BPF_ANY);
    }
    
    
  //  bpf_printk("sys_enter_connect: PID=%d, Comm=%s\n", pid, conn_info.comm);
    return 0;
}
...
int trace_connect_exit(struct sys_exit_connect_args *ctx) {
    ...
    if (addr.sin_family == AF_INET) {
            ...
            u32 ip = bpf_ntohl(addr.sin_addr.s_addr);
            u16 port = bpf_ntohs(addr.sin_port);
            bpf_printk("sys_exit_connect:ADDR=%d.%d.%d.%d:%d PID=%d, Comm=%s\n",
            (ip >> 24) & 0xFF, (ip >> 16) & 0xFF,
            (ip >> 8) & 0xFF, ip & 0xFF,  
            port,
            pid, 
