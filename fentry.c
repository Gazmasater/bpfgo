...
int trace_accept4_entry(struct sys_enter_accept4_args *ctx) {
    ...
    if (bpf_probe_read_kernel(&temp_sock_addr, sizeof(temp_sock_addr), ctx->upeer_sockaddr) == 0) {
        conn_info.sock_addr = &temp_sock_addr; // Привязка указателя
        init_conn_info(pid, &conn_info);      // Инициализация conn_info
        bpf_map_update_elem(&conn_info_map_accept, &pid, &conn_info, BPF_ANY);
        bpf_printk("SERVER accept4 entry: PID=%d, Comm=%s\n", pid, conn_info.comm);
    } else {
        bpf_printk("Failed to read sockaddr in entry for PID=%d\n", pid);
    }
...
int trace_accept4_ret(struct sys_exit_accept4_args *ctx) {
    ...
    conn_info = bpf_map_lookup_elem(&conn_info_map_accept, &pid);
    if (!conn_info) {
    ...
    bpf_map_delete_elem(&conn_info_map_accept, &pid); // Удаляем запись после завершения
    return 0;
}
