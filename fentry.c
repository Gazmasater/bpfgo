...
long ret = ctx->ret; // Получаем результат вызова
    ...
    if (bpf_probe_read(&addr, sizeof(addr), conn_info->sock_addr) != 0) {
        bpf_printk("sys_accept4 Failed to read sockaddr for PID=%d\n", pid);
        return 0;
    }
...
//     if (bpf_probe_read(&addr, sizeof(addr), conn_info->sock_addr) != 0) {
//         bpf_printk("ys_accept4 Failed to read sockaddr for PID=%d\n", pid);
//         return 0;
//     }
...
//     // Если результат отрицательный, значит произошла ошибка
//     if (ret < 0) {
//         bpf_printk("sys_exit_bind  Accept4 failed for PID=%d\n", pid);
...
//     struct conn_info_t *conn_info = bpf_map_lookup_elem(&conn_info_map_bind, &pid); 
//     if (conn_info) 
//     { bpf_printk("kprobe SERVER Bind entry: PID=%d, Comm=%s\n", conn_info->pid, conn_info->comm); }
...
//     if (bpf_probe_read(&addr, sizeof(addr), conn_info->sock_addr) != 0) {
//         bpf_printk("kprobe __sys_bind Failed to read sockaddr for PID=%d\n", pid);
//         return 0;
//     }
...
//         bpf_printk("kretprobe SERVER Accepted connection: PID=%d, Comm=%s, IP=%d.%d.%d.%d, Port=%d\n",
//             conn_info->pid, conn_info->comm,
//             (conn_info->dst_ip >> 24) & 0xFF, (conn_info->dst_ip >> 16) & 0xFF,
//             (conn_info->dst_ip >> 8) & 0xFF, conn_info->dst_ip & 0xFF, conn_info->dport);
//     }
